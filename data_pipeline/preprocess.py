...
def wrangle_scdb_data(
    feather_dir: Path = DATA_PATH / 'feather',
    preprocessed_dir: Path = DATA_PATH / 'preprocessed'
) -> None:
    preprocessed_dir.mkdir(parents=True, exist_ok=True)
    scdb_datasets = {
        'SCDB_Legacy-and-2020r1_caseCentered_Citation': pd.concat(
            [pd.read_feather(feather_dir / scdb_version / f'SCDB_{scdb_version}_caseCentered_Citation.feather')
    ...
    for dataset_name, scdb_dataset in scdb_datasets.items():
        (scdb_dataset.pipe(
                ...
                .pipe(set_dtypes)
                .to_feather(preprocessed_dir / f'{dataset_name}.feather')
        )
